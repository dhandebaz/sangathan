...
- Name: features.spec.ts >> Sangathan Platform Features Page >> 5. Verify clicking on a feature item dynamically updates detail panel / accordion
- Location: tests\e2e\features.spec.ts:134:7
...
Locator: locator('.hidden.lg\\:grid h3').filter({ hasText: 'Donor CRM Database' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.hidden.lg\\:grid h3').filter({ hasText: 'Donor CRM Database' }).first()
