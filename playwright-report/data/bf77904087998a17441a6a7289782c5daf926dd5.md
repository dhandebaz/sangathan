...
- Name: features.spec.ts >> Sangathan Platform Features Page >> 3. Verify deep linking via URL hash values sets correct active tab
- Location: tests\e2e\features.spec.ts:94:7
...
Locator: getByRole('heading', { name: 'Student Unions', exact: true }).filter({ visible: true })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Student Unions', exact: true }).filter({ visible: true })
