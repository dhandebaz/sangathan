  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Decisions & Polls</h1>
        ...
        {polls?.map((poll) => (
          <Link href={`/${lang}/dashboard/polls/${poll.id}`} key={poll.id} className="block group">
            <div className="bg-card p-6 rounded-xl border shadow-sm hover:shadow-md transition-shadow">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    <Badge variant="outline" className={
                      poll.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-muted'
                    }>{poll.status}</Badge>
              ...
              <p className="text-muted-foreground mt-2 text-sm line-clamp-2">{poll.description}</p>
