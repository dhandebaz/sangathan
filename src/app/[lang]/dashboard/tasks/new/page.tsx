...
export default async function NewTaskPage(props: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <div className="mb-6">
        <Link href={`/${lang}/dashboard/tasks`} className="flex items-center text-sm text-muted-foreground hover:text-foreground mb-2">
          <ArrowLeft className="w-4 h-4 mr-1" />
          Back to Tasks
        </Link>
        <h1 className="text-2xl font-bold">Create New Task</h1>
      </div>
      <TaskForm orgId={profile.organisation_id} />
    </div>
  )
}
