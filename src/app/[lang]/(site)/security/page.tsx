  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-6">
      ...
      <div className="prose prose-slate max-w-none text-[var(--text-secondary)] space-y-12">
        <section>
          <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-4">{isHindi ? 'अलगाव की वास्तुकला' : 'Architecture of Isolation'}</h2>
          <p>
            {isHindi
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 not-prose my-6">
             <div className="p-6 bg-[var(--surface)] border border-[var(--border-subtle)] rounded-xl shadow-sm">
                <Lock className="w-8 h-8 text-[var(--accent)] mb-3" />
                <h3 className="font-bold text-lg mb-2 text-[var(--text-primary)]">{isHindi ? 'पंक्ति-स्तर की सुरक्षा (RLS)' : 'Row-Level Security (RLS)'}</h3>
             ...
             <div className="p-6 bg-[var(--surface)] border border-[var(--border-subtle)] rounded-xl shadow-sm">
                <Key className="w-8 h-8 text-[var(--accent)] mb-3" />
                <h3 className="font-bold text-lg mb-2 text-[var(--text-primary)]">{isHindi ? 'भूमिका-आधारित पहुंच' : 'Role-Based Access'}</h3>
                <p className="text-sm text-[var(--text-secondary)]">
                   {isHindi
        ...
        <section>
          <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-4">{isHindi ? 'डेटा सुरक्षा' : 'Data Protection'}</h2>
          <ul className="space-y-4">
             <li className="flex gap-3">
                <Server className="w-6 h-6 text-[var(--text-secondary)] shrink-0" />
                <div>
                   <strong className="text-[var(--text-primary)]">{isHindi ? 'आराम और पारगमन पर एन्क्रिप्शन:' : 'Encryption at Rest & Transit:'}</strong> {isHindi ? 'सभी डेटा हमारी डिस्क पर संग्रहीत होने और आपके डिवाइस और हमारे सर्वर (TLS 1.3) के बीच यात्रा करते समय एन्क्रिप्ट किया जाता है।' : 'All data is encrypted while stored on our disks and while traveling between your device and our servers (TLS 1.3).'}
                </div>
             </li>
             <li className="flex gap-3">
                <EyeOff className="w-6 h-6 text-[var(--text-secondary)] shrink-0" />
                <div>
                   <strong className="text-[var(--text-primary)]">{isHindi ? 'सॉफ्ट डिलीशन:' : 'Soft Deletion:'}</strong> {isHindi ? 'जब आप डेटा हटाते हैं, तो यह आकस्मिक डेटा हानि या दुर्भावनापूर्ण पोंछने को रोकने के लिए सुरक्षा अवधि (जैसे, 7 दिन) के लिए "सॉफ्ट-डिलीट" स्थिति में प्रवेश करता है।' : 'When you delete data, it enters a "soft-delete" state for a safety period (e.g., 7 days) to prevent accidental data loss or malicious wiping.'}
                </div>
             </li>
             <li className="flex gap-3">
                <AlertTriangle className="w-6 h-6 text-[var(--text-secondary)] shrink-0" />
                <div>
                   <strong className="text-[var(--text-primary)]">{isHindi ? 'दुरुपयोग संरक्षण:' : 'Abuse Protection:'}</strong> {isHindi ? 'हम आपके खाते के खिलाफ ब्रूट-फोर्स हमलों और "क्रेडेंशियल स्टफिंग" प्रयासों को रोकने के लिए बुद्धिमान दर सीमित करने का उपयोग करते हैं।' : 'We use intelligent rate limiting to block brute-force attacks and "credential stuffing" attempts against your account.'}
                </div>
             </li>
          </ul>
        </section>
        ...
        <section>
          <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-4">{isHindi ? 'सत्यापित प्रशासन' : 'Verified Administration'}</h2>
          <p>
            {isHindi
        ...
        <section>
          <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-4">{isHindi ? 'तृतीय-पक्ष बुनियादी ढांचा' : 'Third-Party Infrastructure'}</h2>
          <p>
            {isHindi
          ...
          <ul className="list-disc pl-5 space-y-2">
             <li><strong className="text-[var(--text-primary)]">Supabase:</strong> Database & Authentication</li>
             <li><strong className="text-[var(--text-primary)]">Firebase:</strong> SMS Verification</li>
             <li><strong className="text-[var(--text-primary)]">Vercel:</strong> Global Edge Network</li>
             <li><strong className="text-[var(--text-primary)]">Razorpay:</strong> PCI-DSS Compliant Payments</li>
          </ul>
        </section>
      </div>
    </div>
